...
- (void)processImageWithData:(NSData *)pickerImageData API_AVAILABLE(ios(14)) {
  ...
  // maxWidth and maxHeight are used inside this method ONLY for GIF images.
  // For other formats, the 'image' argument is already scaled (if needed) by the code above.
  // If requestFullMetadata is NO, pass nil for originalImageData to strip metadata.
  NSData *dataForMetadata = self.requestFullMetadata ? pickerImageData : nil;
  NSString *savedPath =
      [FLTImagePickerPhotoAssetUtil saveImageWithOriginalImageData:dataForMetadata
                                                             image:localImage
                                                          maxWidth:self.maxWidth
                                                         maxHeight:self.maxHeight
